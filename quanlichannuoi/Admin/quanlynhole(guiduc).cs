...
{
    public partial class quanlynhole_guiduc_ : UserControl
    {
        ...
        public quanlynhole_guiduc_()
        {
            InitializeComponent();
            LoadComboBoxes(); // Nạp dữ liệu cho ComboBox
        ...
        private void quanlynhole_guiduc__Load(object sender, EventArgs e)
        {
        ...
        VALUES (@NameFacilities, @NameS, @Address, @Phone, @Email, @DistrictId, @CommuneId, @ProductTypes, @Scale)";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        // Giá trị mặc định cho Scale
                        string scaleValue = comboBoxScale.SelectedItem?.ToString();
                        if (string.IsNullOrEmpty(scaleValue))
                        {
                            scaleValue = "Nhỏ"; // Giá trị mặc định
                        }
                        cmd.Parameters.AddWithValue("@Scale", scaleValue);
